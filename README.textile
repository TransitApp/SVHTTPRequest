__Note: SVHTTPRequest now uses Automatic Reference Counting (ARC) and uses solely NSJSONSerialization to parse the JSON responses (it's therefore only compatible with iOS 5 and Mac OS X Lion). Find the last MRC (also uses JSONKit so it's compatible with iOS 4) commit on the "master-mrc":https://github.com/samvermette/SVHTTPRequest/tree/master-mrc branch.__
