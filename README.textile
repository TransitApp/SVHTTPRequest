...
SVHTTPRequest is a simple and extremely straightforward way to communicate with RESTful web APIs. It's a simpler and cleaner alternative to bulky "ASIHTTPRequest":https://github.com/pokeb/asi-http-request/tree, "AFNetworking":https://github.com/AFNetworking/AFNetworking and "RESTKit":https://github.com/RestKit/RestKit. It is blocked-based, uses @NSURLConnection@ as well as @NSJSONSerialization@ (if available, otherwise "JSONKit":https://github.com/johnezang/JSONKit - included in package) to automatically parse JSON responses.
...
If most of your requests are made to the same API endpoint, use @SVHTTPClient@ and set parameters (@basePath@, @cachePolicy@, @sendParametersAsJSON@) that will be used for each request made:
...
In addition, SVHTTPClient allows you to sign all outgoing requests for "basic authentication":http://en.wikipedia.org/wiki/Basic_access_authentication using @setBasicAuthWithUsername:password:@.

h3. Making individual requests

You can still make individual requests using similar convenience methods:
...
As always, you can replace @GET@ with @POST@, @PUT@ or @DELETE@. If you would like to set advanced attributes (such as @cachePolicy@, @sendParametersAsJSON@) to an individual request, you'll need to alloc/init the request, set the attributes, and then call @start@:
